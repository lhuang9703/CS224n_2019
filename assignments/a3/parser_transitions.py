...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            depen = (self.stack[-1], self.stack[-2])
            self.stack.pop(-2)
            self.dependencies.append(depen)
        elif transition == "RA":
            depen = (self.stack[-2], self.stack[-1])
            self.stack.pop(-1)
            self.dependencies.append(depen)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for partial_parse, transition in zip(miniBatch, miniBatch_transitions):
            partial_parse.parse_step(transition)
            if len(partial_parse.buffer) == 0 and len(partial_parse.stack) == 1:
                unfinished_parses.remove(partial_parse)
    dependencies = [item.dependencies for item in partial_parses]
    ### END YOUR CODE
